...
// ─────────────────────────────────────────────────────────────────────────────
// Route sampling — ROUTES_SAMPLE / ROUTES_SEED
//
// setup() searches EVERY route it is given, sequentially, once per scenario side. With the full
// table and SCENARIO=both that is 2 x ROUTES.length calls to search-service before a single
// journey runs. Experiment 01 wants that breadth — spreading load is the point, so inventory
// never becomes the bottleneck. The fault-injection experiments (04, 05, 06) do not: they fire
// small batches and only need enough stock to cover them, so they pay a long warm-up for
// variety they never use.
//
// `ROUTES_SAMPLE=n` takes n routes instead of all of them. Unset = the whole table, so
// Experiment 01 and anything else is unaffected.
//
// WHY THE SAMPLE IS SEEDED. `Math.random()` cannot be seeded in k6, so a plain random pick
// would make two runs of the same experiment touch different routes — and the runs recorded in
// RESULTS.md would stop being comparable. A tiny deterministic PRNG keeps the default sample
// stable across runs; pass a different ROUTES_SEED to deliberately draw another one.
//
// SIZING IT. Too small concentrates the batch onto few resources and recreates Experiment 02's
// contention by accident: bookings compete for the same seats and get rejected, which pollutes
// a resilience run with FAILED states that did not come from the injected fault. Keep
// `batch size / sample size` well under the per-flight capacity (a few hundred seats).
// ─────────────────────────────────────────────────────────────────────────────

/** Mulberry32 — 4-line deterministic PRNG. Same seed, same sequence, every run and platform. */
...
/**
 * `n` routes drawn from the table without replacement, or the whole table when `n` is unset,
 * zero, or already >= the table size. Partial Fisher-Yates: shuffles only what it needs.
 */
function sampleRoutes(routes, n, seed) {
  if (!n || n <= 0 || n >= routes.length) return routes;
  const pool = routes.slice();
...
const ROUTES_SAMPLE = parseInt(env('ROUTES_SAMPLE', '0'), 10);
const ROUTES_SEED = parseInt(env('ROUTES_SEED', '20260726'), 10);
const ACTIVE_ROUTES = sampleRoutes(ROUTES, ROUTES_SAMPLE, ROUTES_SEED);
...
export const CONFIG = {
  ...
  search: {
    // The full table, or a seeded sample of it when ROUTES_SAMPLE is set (see above).
    routes: ACTIVE_ROUTES,                            // dates come per-route (departureDate)
    routesTotal: ROUTES.length,                       // so setup() can report "n of N"
    routesSeed: ROUTES_SEED,                          // reported too: a sample is only reproducible with its seed
