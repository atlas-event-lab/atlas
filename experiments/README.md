...
| 08 | [Cluster Portability](./08-cluster-portability) | Scalability | "12 vCPU" is not a portable unit — identical nominal clusters serve materially different throughput |
...
|-------|---------------------|
| `KEYCLOAK_CLIENT_SECRET` | generated by Keycloak for the `atlas-loadtest` client (confidential, *Direct Access Grants* on) — created by `deploy/platform/keycloak/realm-import.yaml`, Runbook Step 5b |
...
> **`.env` holds no secrets.** Keep it for the non-sensitive knobs (`SCENARIO`, `TARGET_RPS`,
> `LOADTEST_USER_COUNT`, …) and let `cluster-credentials.sh` supply the rest. Load the
> credentials **after** `.env` so an old value in the file cannot shadow the live one.
>
> **Deleted `temp-admin`?** Runbook Step 5a tells you to, once you have a permanent admin.
> After that, export `KEYCLOAK_ADMIN_USER` and `KEYCLOAK_ADMIN_PASSWORD` yourself before
> running the script — it says so explicitly if it cannot find the bootstrap secret.
