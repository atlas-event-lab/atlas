#!/usr/bin/env bash
# apps-idle.sh — scale the business apps to 0 while KEEPING the cluster (and platform:
...
#
# NOTE: this does NOT save credits — nodes stay powered on. For credit savings use
# cluster-down.sh instead. Resume with apps-resume.sh.
#
...
# its HPA and scales them straight back up after `scale ... --replicas=0`. apps-resume.sh removes
# the pin.
set -euo pipefail
NS="${NS:-atlas-apps}"
...
echo ">> Idle. Deployments remain defined (image tags preserved). Resume: ./apps-resume.sh"
